...
class EnsemblePredictor:
    ...
    def train(self, X, y, feature_names=None, cv_folds=5, match_dates=None, league_ids=None):
        """
        ...
        league_ids: optional list of league_id ints — used to apply league-tier
                    weights so top leagues (Premier League, La Liga etc.) are
                    learned from more heavily than lower divisions.
        """
        import datetime

        # ── League name lookup built from DB league_ids ───────────────────
        # We map league_id → tier weight using the league names we know.
        # Tier 1 (3.0): Top 5 + other strong European domestic leagues
        # Tier 2 (2.0): UEFA competitions + solid mid-tier leagues
        # Tier 3 (1.0): Default (lower divisions, other leagues)
        _TIER1_NAMES = {
        ...
        # Build league_id → weight map if league_ids provided
        ...
        if league_ids is not None:
            ...
            try:
                ...
                if unique_lids:
                    ...
                    _cur.execute(f"SELECT id, name FROM leagues WHERE id IN ({placeholders})", unique_lids)
        ...
        # Apply league-tier weights if league_ids provided.
        # Top leagues (Premier League, La Liga etc.) get 3× weight — the model
        # learns harder from high-quality predictable data. Lower divisions get
        # 1× weight. This corrects the accuracy drop caused by 41k matches
        # including many unpredictable lower-league fixtures.
        if league_ids is not None and len(league_ids) == len(y) and _league_weights:
